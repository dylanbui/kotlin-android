...
public class DbMaterialSpinner extends TextView {

  private OnNothingSelectedListener onNothingSelectedListener;
  private OnItemSelectedListener onItemSelectedListener;
  private DbMaterialSpinnerBaseAdapter adapter;
  ...
  public DbMaterialSpinner(Context context) {
    super(context);
    init(context, null);
  }

  public DbMaterialSpinner(Context context, AttributeSet attrs) {
    super(context, attrs);
    init(context, attrs);
  }

  public DbMaterialSpinner(Context context, AttributeSet attrs, int defStyleAttr) {
    super(context, attrs, defStyleAttr);
    init(context, attrs);
  }

  private void init(Context context, AttributeSet attrs) {
    TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.DbMaterialSpinner);
    ...
    defaultPaddingLeft = defaultPaddingRight =
            defaultPaddingBottom = defaultPaddingTop = resources.getDimensionPixelSize(R.dimen.dbms__padding_top);
    if (rtl) {
      defaultPaddingRight = resources.getDimensionPixelSize(R.dimen.dbms__padding_left);
    } else {
      defaultPaddingLeft = resources.getDimensionPixelSize(R.dimen.dbms__padding_left);
    }
    defaultPopupPaddingLeft =
            defaultPopupPaddingRight = resources.getDimensionPixelSize(R.dimen.dbms__popup_padding_left);
    defaultPopupPaddingTop = defaultPopupPaddingBottom = resources.getDimensionPixelSize(R.dimen.dbms__popup_padding_top);

    try {
      backgroundColor = ta.getColor(R.styleable.DbMaterialSpinner_dbms_background_color, Color.WHITE);
      backgroundSelector = ta.getResourceId(R.styleable.DbMaterialSpinner_dbms_background_selector, 0);
      textColor = ta.getColor(R.styleable.DbMaterialSpinner_dbms_text_color, defaultColor);
      hintColor = ta.getColor(R.styleable.DbMaterialSpinner_dbms_hint_color, defaultColor);
      arrowColor = ta.getColor(R.styleable.DbMaterialSpinner_dbms_arrow_tint, textColor);
      hideArrow = ta.getBoolean(R.styleable.DbMaterialSpinner_dbms_hide_arrow, false);
      arrowDrawableResId = ta.getResourceId(R.styleable.DbMaterialSpinner_dbms_arrow_drawble, R.drawable.dbms__arrow);
      hintText = ta.getString(R.styleable.DbMaterialSpinner_dbms_hint) == null ? ""
              : ta.getString(R.styleable.DbMaterialSpinner_dbms_hint);
      popupWindowMaxHeight = ta.getDimensionPixelSize(R.styleable.DbMaterialSpinner_dbms_dropdown_max_height, 0);
      popupWindowHeight = ta.getLayoutDimension(R.styleable.DbMaterialSpinner_dbms_dropdown_height,
              WindowManager.LayoutParams.WRAP_CONTENT);
      paddingTop = ta.getDimensionPixelSize(R.styleable.DbMaterialSpinner_dbms_padding_top, defaultPaddingTop);
      paddingLeft = ta.getDimensionPixelSize(R.styleable.DbMaterialSpinner_dbms_padding_left, defaultPaddingLeft);
      paddingBottom = ta.getDimensionPixelSize(R.styleable.DbMaterialSpinner_dbms_padding_bottom, defaultPaddingBottom);
      paddingRight = ta.getDimensionPixelSize(R.styleable.DbMaterialSpinner_dbms_padding_right, defaultPaddingRight);
      popupPaddingTop =
              ta.getDimensionPixelSize(R.styleable.DbMaterialSpinner_dbms_popup_padding_top, defaultPopupPaddingTop);
      popupPaddingLeft =
              ta.getDimensionPixelSize(R.styleable.DbMaterialSpinner_dbms_popup_padding_left, defaultPopupPaddingLeft);
      popupPaddingBottom =
              ta.getDimensionPixelSize(R.styleable.DbMaterialSpinner_dbms_popup_padding_bottom, defaultPopupPaddingBottom);
      popupPaddingRight =
              ta.getDimensionPixelSize(R.styleable.DbMaterialSpinner_dbms_popup_padding_right, defaultPopupPaddingRight);
    ...
    setBackgroundResource(R.drawable.dbms__selector);
    ...
    if (!hideArrow) {
      arrowDrawable = Utils.getDrawable(context,arrowDrawableResId).mutate();
    ...
    listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
      ...
      @Override public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        if (onItemSelectedListener != null) {
          ...
          onItemSelectedListener.onItemSelected(DbMaterialSpinner.this, position, id, item);
        }
      }
    });
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      ...
      popupWindow.setBackgroundDrawable(Utils.getDrawable(context,R.drawable.dbms__drawable));
    } else {
      popupWindow.setBackgroundDrawable(Utils.getDrawable(context, R.drawable.dbms__drop_down_shadow));
    }
    ...
    popupWindow.setOnDismissListener(new PopupWindow.OnDismissListener() {
      ...
      @Override public void onDismiss() {
        if (nothingSelected && onNothingSelectedListener != null) {
          onNothingSelectedListener.onNothingSelected(DbMaterialSpinner.this);
        }
        if (!hideArrow) {
          animateArrow(false);
        }
...
//          setAnimation(animation);
          ValueAnimator fadeAnim = ObjectAnimator.ofFloat(this, "alpha", 0.3f, 1f);
  ...
  private void setAdapterInternal(@NonNull DbMaterialSpinnerBaseAdapter adapter) {
  ...
  private int calculatePopupWindowHeight() {
    ...
    float itemHeight = getResources().getDimension(R.dimen.dbms__item_height);
  ...
  public interface OnItemSelectedListener<T> {
     ...
     * item.</p>
     *
     * @param view The {@link DbMaterialSpinner} view
     * @param position The position of the view in the adapter
     * @param id The row id of the item that is selected
     * @param item The selected item
     */
    void onItemSelected(DbMaterialSpinner view, int position, long id, T item);
  }
  ...
  public interface OnNothingSelectedListener {
     ...
     *
     * @param spinner the {@link DbMaterialSpinner}
     */
    void onNothingSelected(DbMaterialSpinner spinner);
  }
}