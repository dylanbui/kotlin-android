...
// Base on: https://github.com/TheKhaeng/view-group-savedstate-example


abstract class DbBaseViewGroup : FrameLayout {
     ...
     */
    protected fun onSaveChildInstanceState(ss: DbChildSavedState): Parcelable {
    ...
    private fun onRestoreChildInstanceState(ss: DbChildSavedState) {
        for (i in 0 until childCount) {
            val id = getChildAt(i).id
            if (id != 0) {
    ...
    abstract class DbChildSavedState : View.BaseSavedState {
