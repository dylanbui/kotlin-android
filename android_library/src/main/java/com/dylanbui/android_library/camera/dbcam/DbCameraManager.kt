...
interface DbCameraManagerCallback {
    fun onCameraReady()
    fun onPicture(image: Image)
 ...
 */
internal class DbCompareSizesByArea : Comparator<Size> {
...
public class DbCameraManager {
    ...
    private lateinit var cameraCallback: DbCameraManagerCallback
    ...
    /**
     * An [DbAutoFitTextureView] for camera preview.
     */
    ...
    private val stateCallback = object : CameraDevice.StateCallback() {
        ...
        override fun onOpened(varCameraDevice: CameraDevice) {
            cameraOpenCloseLock.release()
            // this@Camera2BasicFragment.cameraDevice = cameraDevice
            cameraDevice = varCameraDevice
            createCameraPreviewSession()
        }

        override fun onDisconnected(varCameraDevice: CameraDevice) {
            cameraOpenCloseLock.release()
            varCameraDevice.close()
        ...
        override fun onError(varCameraDevice: CameraDevice, error: Int) {
            onDisconnected(varCameraDevice)
    ...
    private val onImageAvailableListener = ImageReader.OnImageAvailableListener {
        // backgroundHandler?.post(DbImageSaver(it.acquireNextImage(), file))
        // -- Run on uithread --
        (context as Activity).runOnUiThread {
            cameraCallback.onPicture(it.acquireLatestImage())
        }
    }
...
//        TODO("pai tao file chua")
//        file = File(activity.getExternalFilesDir(null), PIC_FILE_NAME)
//    }

    constructor(context: Context, textureView:  DbAutoFitTextureView) {
     ...
     */
    fun setCameraCallback(cameraCallback: DbCameraManagerCallback) {
        ...
        try {
            for (cameraId in manager.cameraIdList) {
                ...
                val largest = Collections.max(
                    Arrays.asList(*map.getOutputSizes(ImageFormat.JPEG)), DbCompareSizesByArea())
                ...
                // Cau hinh default camera ORIENTATION
//                if (resources.configuration.orientation == Configuration.ORIENTATION_LANDSCAPE) {
//                    textureView.setAspectRatio(previewSize.width, previewSize.height)
//                } else {
//                    textureView.setAspectRatio(previewSize.height, previewSize.width)
//                }
                // DucBui: Auto default camera ORIENTATION = PORTRAIT
                textureView.setAspectRatio(previewSize.height, previewSize.width)
...
//    }
    ...
    companion object {
        ...
        @JvmStatic private fun chooseOptimalSize(
            ...
            if (bigEnough.size > 0) {
                return Collections.min(bigEnough, DbCompareSizesByArea())
            } else if (notBigEnough.size > 0) {
                return Collections.max(notBigEnough, DbCompareSizesByArea())
            } else {
